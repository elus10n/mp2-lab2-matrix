...
{
	const int size = 4;
	TDynamicVector<int> tmp(size), tmp1(size), rez(size);
	tmp = tmp + 2;
...
{
	const int size = 4;
	TDynamicVector<int> tmp(size), tmp1(size);
	for (int i = 0; i < size; i++)//{1,2,3,4}
		tmp[i] = i + 1;
	for (int i = 0; i < size; i++)//{2,3,4,5}
		tmp1[i] = i + 2;
...
TEST(TDynamicVector, cant_multiply_vectors_with_not_equal_size)
{
	...
	TDynamicVector<int> tmp(size1), tmp1(size2);
	ASSERT_ANY_THROW(tmp * tmp1);
}
